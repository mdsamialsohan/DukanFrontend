...
const PickUp = () => {
    ...
    const AccAdd = `${apiAdd}/UserInfo`;
    const TransferAdd = `${apiAdd}/PickUp`;
    ...
    const AccOption =availableAcc.map((User) => ({
        value: User.id,
        label: `${User.name} -
            ${User.Cash}`,
        due: User.Cash,
                                ...
                                <form onSubmit={handleSubmit}>
                                    <div className="row">
                                        <div className="form-group col-md-4">
                                            ...
                                            <Select
                                                options={AccOption}
                                                value={AccOption.find((option) => option.value === AccID)}
                                                onChange={(selectedOption) => (setAccID(selectedOption.value))}
                                            />
                                            {AccID === '' && isBlank && <p className="text-danger">cannot be empty</p>}
